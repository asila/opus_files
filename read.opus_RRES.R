...
# 'Modified: 7/February/2018
# 'Objective: Modified from the exisiting read.opus to convert scans from RRES lab
...
opus <- function(file.name, sp = NULL,
	...
	spec.env()
    	...
    	## Read metainfo
      	...
      	if(!class(.Last.value)[1] == "try-error"){
		 	...
		 	ins <- readRaw(file.name, offset = ins[length(ins)]+7, nbytes = 3, human = 
		 	
		 	"char", size = 1, endian = "little")
		    ...
		    src <- readRaw(file.name, offset = src[length(src)]+4, nbytes = 3, human = 
		    ...
		    bms <- readRaw(file.name, offset = bms[length(bms)] + 4, nbytes = 13, human = "char", size = 1,
		    
		    endian = "little")
		    
		    bms <- blockString(bms)
		    	
         ## Wavenumbers for MIR spectra from Tensor are assigned prefix "m", MIR spectra from Alpha
         
         #prefixed "a"; for NIR MPA "n"
         
        		if(instr.range == "ten-off"){ instr.range == "ten-mir"} ## AS: Old ten-mir written as 
        		
        		#tensor-27
        		
		pref <- ifelse(instr.range == "ten-mir", "m", ifelse(instr.range == "alp-mir", "a",
		
		ifelse(instr.range == "mpa-nir", "n", "X")))
		
			if(speclib=="ICRAF"){
				 
          			if(instr.range=="ten-mir"){
          				
					  wb <- rev(seq(get("icraf.htsxt", spec.opts)[3], get("icraf.htsxt", 
					  
					  spec.opts)[2], (get("icraf.htsxt", spec.opts)[2]-get("icraf.htsxt", 
					  
					  spec.opts)[3])/(get("icraf.htsxt", spec.opts)[1]-1)))
				  								}
				  	...
				  	if(instr.range == "alp-mir"){
						...
						wb <- rev(seq(get("icraf.alpha", spec.opts)[3], get("icraf.alpha",
						
						spec.opts)[2], (get("icraf.alpha", spec.opts)[2]-get("icraf.alpha",
						
						spec.opts)[3])/(get("icraf.alpha", spec.opts)[1]-1)))
						 
          								}
          								
          			if(bms == "ZnSe"){
          					
          				wb <- rev(seq(get("icraf.alpha.znse", spec.opts)[3], get("icraf.alpha.znse",
          				
          				spec.opts)[2], (get("icraf.alpha.znse", spec.opts)[2]-get("icraf.alpha.znse",
          				 
          				spec.opts)[3])/(get("icraf.alpha.znse", spec.opts)[1]-1)))}
          				  
 				 						}

			    		if(instr.range=="mpa-nir"){
			    				
						wb <- rev(seq(get("icraf.mpa", spec.opts)[3], get("icraf.mpa",
						
						spec.opts)[2], (get("icraf.mpa", spec.opts)[2]-get("icraf.mpa",
						
						spec.opts)[3])/(get("icraf.mpa", spec.opts)[1]-1)))
 				  								  }
	 	    						}
	 	    						
	 	    		if(!(instr.range == "ten-mir"|instr.range == "alp-mir"|instr.range == 
	 	    			
	 	    			"mpa-nir"|
	 	    			
	 	    			instr.range == "ten-off"|bms == "ZnSe"))
	 	    			
	 	    			{ stop("Unknown file format. See '?read.opus' for more info.") }  
			...
			## Get positions where the following parameters are found in the file  
     		...
     		npt0 <- grepRaw(codes[8],pr,all = TRUE) + 3
			...
			## calculate end and start of each block:
 			...
 			offs <- sapply(1:length(end), function(x){end[x]})
     		...
     		# Extract spectrum metadata
  	 		...
  	 		ZFF <- readRaw(file.name, offset = z, nbytes = 4, human="int",
  	 		
  	 		size = 2)[[5]][1]
  	 		
  	 		RES <- readRaw(file.name, offset = re, nbytes = 4, human = "int",
  	 		
  	 		size = 2)[[5]][1]
  	 		
  			snm.lab.material <- blockString(readRaw(file.name, offset = snm,
  			...
  			if(!nzchar(snm.lab.material)){
  		 	  ...
  		 	  SSN <- ""
  		  	  ...
  		  	  warning("Product name not found inside OPUS file...")
            		...
            		as.vector(strsplit(snm.lab.material,";"))[[1]]
            		
           			SSN <- paste0(snm.lab.material[3], snm.lab.material[1])
           			
           			Material <- snm.lab.material[4]
   			  															} 
       		 		
       		 		
       		 		
       		 		snm.lab.material <- blockString(readRaw(file.name, offset
       		 		
       		 		= snm, nbytes = 22, human = "char",
       		 		
       		 		size = 1, endian = "little"))
       		 		
       		 		if(!length(grep(snm.lab.material, pattern="_"))==0){
       		 				 	
          	  				SSN <- gsub("_", "", snm.lab.material)
		...
		SSN <- paste0(tolower(substr(SSN,1,3)), substr(SSN,5,20))
		
  	 	Scandate <- blockString(readRaw(file.name, offset = dat, nbytes = 10, human = "char", size = 1, 
  	 	
  	 	endian = "little"))
  	 	
  	 	Scantime <- blockString(readRaw(file.name, offset = tim[2]-4, nbytes = 8, human = "char", size = 1,
  	 	
  	 	endian = "little"))
  	 	
  	 	Scandate <- paste(Scandate,Scantime)
  	 	
  		LWN <- round(readRaw(file.name, offset = lwn, nbytes = 8, human="real", size = 8)[[5]][1],1)
		...
		## Get number of data points for each spectra data block
  	 	...
  	 	NPT0 <- readRaw(file.name, offset=npt0[1], nbytes=12, human="int", size=4)[[5]][2]; 
  	 	...
  	 	#Start reading available data blocks
			...
			for (f in 6:length(offs)){ 
				
				offs.f <- offs[f]
				
			try(opus.p <-readRaw(file.name,width=NULL,offset=offs.f-4,nbytes=nbytes.f,
			
			human="real",size=4,endian="little"), silent = TRUE)
		...
		#Determine valid data block to read
			...
			fs <- as.vector(which(mxsa[,1] > -0.1 & (mxsa[,2] > 1))[1])
			
			if(is.na(fs)== TRUE){
			fs <- as.vector(which(mxsa[,1] > -0.2 & (mxsa[,2] > 1))[1])	
			}
		...
		try(opus.p <-readRaw(file.name,width=NULL,offset=offs.f-4,nbytes=nbytes.f,human="real",
		
		size=4,endian="little"), silent=TRUE)
	    	...
	    	## Add meta ID and create data.frames:
	    	
		samples <- data.frame(SAMPLEID=spectrum.meta[1], Material=spectrum.meta[2], 
		
		Zero.Filing=spectrum.meta[4], Resolution=spectrum.meta[5], LWN=spectrum.meta[6],
        ...
        names(spec)<-c("SAMPLEID","Material","Datetime","Zero.filling","Resolution","LWN"
        
        ,paste0(pref,round(Wavenumbers,1)))
